...
cprintf('[10/255,30/255,150/255]','| |                                                  | |\n')
cprintf('[10/255,30/255,150/255]','| |                                                  | |\n')
cprintf('[10/255,30/255,150/255]','| |                  TANKEF v1.0                     | |\n')
cprintf('[10/255,30/255,150/255]','| |                                                  | |\n')
...
E=0.8e9;              % Youngs Modulus [Pa]
v=0.46;               % Poissons ratio
...
H=3.1;    % Hight of the tank from 0 [m]. The thickness of the bottom must be considered
...
% %% Weight of the fluid (Comment lines 166 to 184 so this force is not considered)
...
Fonwall_L=pi*((sDBT(1:end-1,1)+sDBT(2:end,1))*0.5).*((-sDBT(1:end-1,2)+sDBT(2:end,2)));
Fonwall_L(end)=0; %Keep the force in the final node = to 0 to respect equations
for u=1:size(dF_x_L,1)-1
F(dof_L_Y_left(u))=(F(dof_L_Y_left(u))-Fonwall_L(u));
F(dof_L_Y_left(u+1))=(F(dof_L_Y_left(u+1))-Fonwall_L(u));%Negative because of direction
end
...
Fonwall_R=pi*((sDBT(1:end-1,1)+sDBT(2:end,1))*0.5).*((-sDBT(1:end-1,2)+sDBT(2:end,2)));
Fonwall_R(end)=0; %Keep the force in the final node = to 0 to respect equations
for u=1:size(dF_x_R,1)-1
F(dof_L_Y_right(u))=(F(dof_L_Y_right(u))+Fonwall_R(u));
F(dof_L_Y_right(u+1))=(F(dof_L_Y_right(u+1))+Fonwall_R(u));%Negative because of direction
end

%% COMPUTE STRAIN: THIS REQUIRES INVERSION OF K
cprintf('*[210/255,0,0]','Computing Nodal Displacements. Please Wait.\n')
Q=K\F; % Compute strain
%% GET DISPLACEMENTS
